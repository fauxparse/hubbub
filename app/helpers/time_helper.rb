  ...
  def graph_recent_activity(times, end_date = nil)
    ...
    returning "" do |result|
      ...
      hours_per_day.each do |d, h|
        result << content_tag(:li, link_to(content_tag(:span, d.day, :class => :label) + content_tag(:span, "(#{h.to_s(:fraction)})", :class => :count, :style => "height:#{h.to_i * 100 / max_hours}%"), "#", :title => d.to_s))
      end
      result << "</ul>"
    end
  end
